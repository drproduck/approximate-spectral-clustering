...
%                    Default is 200. 
% 'clusterMaxRestart' - when 'Start' is 'cluster'. Maximum number of
%                    restarts of the preliminary clustering phase.
%                    Default is 20;
% 'clusterSample' - Percentage of samples to use for option 'Start' =
%                   'cluster'. Default is 10% (0.1 when input)
%
%
%    Examples:
%
...
pnames = {   'distance' 'start'   'maxiter'  'replicates' 'onlinephase' 'clustermaxiter' 'clustermaxrestart' 'clustersample'};
dflts =  {'sqeuclidean' 'sample'       []        []        'off'              []                []              []          };
[eid,errmsg,distance,start,maxit,reps,online,clustermaxit,clustermaxrestart,clustersample] = getargs(pnames, dflts, varargin{:});
if ~isempty(eid)
...
if ischar(start)
    ...
    % if number of cluster samples less than 5 times centroids, change from
    % 'cluster' to 'sample' (?)
    if j == 2
        if floor(clustersample*n) < 5*k
            j = 1;
        end
    end
    start = startNames{j};
...
% The number of maximum restarts and maximum iterations and sample percentage
% for preliminary clustering phase on random 10% subsamples default to 20
% and 200 and 0.1
if isempty(clustermaxrestart)
    clustermaxrestart = 20;
end
...
for t=1:reps
    switch start
        ...
        case 'cluster'
            Xsubset = X(randsample(n,floor(clustersample*n)),:);
            [~, center] = litekmeans(Xsubset, k, varargin{:}, 'start','sample', 'Replicates', clustermaxrestart, 'MaxIter', clustermaxit, 'clustersample', clustersample);
        case 'numeric'
    end
