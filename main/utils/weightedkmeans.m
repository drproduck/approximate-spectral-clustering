function [label, center, bCon, sumD, D] = weightedkmeans(X, w, k, varargin)
    ...
    switch distance
        case 'sqeuclidean'
            while any(label ~= last) && it<maxit
                ...
                center = full((E*sparse(1:k, 1:k, 1./sum(E,1)))'*X);    % compute center of each cluster
            ...
            if isempty(bestlabel)
                ...
                if reps>1
                    if it>=maxit
                        ...
                        bb = full(sum(center.*center,2));
                        ab = full(X*center');
                        D = bsxfun(@times, bsxfun(@plus,aa,bb') - 2*ab, w);
                        D(D<0) = 0;
                    else
                        aa = full(sum(X.*X,2));
                        D = aa(:,ones(1,k)) + D;
